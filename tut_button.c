...
int main (void)
{
  ...
  fSel =  1;  // 2   // GPIO 23 lives in register 2 (GPFSEL2)
  shift =  6; // 9  // GPIO 23 sits in slot 3 of register 2, thus shift by 3*3 (3 bits per pin)
  ...
  *(gpio + fSel) = (*(gpio + fSel) & ~(7 << shift)) | (1 << shift) ;  // Sets bits to one = output
  ...
  shift =  18; // 12 // GPIO 24 sits in slot 4 of register 3, thus shift by 4*3 (3 bits per pin)
